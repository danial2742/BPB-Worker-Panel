...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.kv_configs.get("proxySettings", {type: 'json'});
        warpConfigs = await env.kv_configs.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.kv_configs.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
            throw new Error(`An error occurred while getting current KV settings - ${error}`);
        }
    } else {
        await env.kv_configs.delete('warpConfigs');
        newSettings = null;
    }
    ...
    try {    
        await env.kv_configs.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
