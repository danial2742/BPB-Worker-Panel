...
async function generateJWTToken (request, env) {
    ...
    const savedPass = await env.kv_configs.get('pwd');
    if (password !== savedPass) return new Response('Method Not Allowed', { status: 405 });
    let secretKey = await env.kv_configs.get('secretKey');
    if (!secretKey) {
        secretKey = generateSecretKey();
        await env.kv_configs.put('secretKey', secretKey);
    }
...
export async function Authenticate (request, env) {
    try {
        const secretKey = await env.kv_configs.get('secretKey');
        const secret = new TextEncoder().encode(secretKey);
...
export async function resetPassword(request, env) {
    ...
    const oldPwd = await env.kv_configs.get('pwd');
    ...
    await env.kv_configs.put('pwd', newPwd);
    return new Response('Success', {
        status: 200,
        headers: {
